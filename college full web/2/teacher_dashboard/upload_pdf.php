...
<div id="removeNotificationModal" class="modal fade zoomIn" tabindex="-1" aria-hidden="true">
 ...
 <div class="col-lg-6">
                                    <div class="p-lg-5 p-4">
                                        <div>
                                            <h5 class="text-primary">Upload PDF</h5>
                                        </div>
                        ...
                        ?>
                     <form action="upload_pdf.php" method="post" class="new-added-form" enctype="multipart/form-data">
                        <div class="row">
                           ...
                           <div class="col-6 col-12 form-group">
                              <select id="branch" required name="branch" class="form-control" style="background-color:#FBE47B;">
                                 <option value="">---- Select Branch---- </option>
                                 <option value="BBA">BTECH</option>
                                 <option value="BCA">MTECH</option>
                                 <option value="BSC">MCA</option>
                              </select>
                           </div><br><br><br>
                           <div class="col-6 col-12 form-group">
                        ...
                        $subject = mysqli_real_escape_string($con, $_POST['subject']);
                            ...
                            if(in_array($file_extension, $allowed_extensions)){
                                ...
                                if(move_uploaded_file($file_tmp, $target)){
                                    // Update the database
                                    $query = mysqli_query($con, "INSERT INTO `pdf`(`userid`, `name`, `branch`, `subject`, `pdf`) 
                        VALUES ('$userid','$name','$branch','$subject','$file')");
                                    
                                    if($query){
                                        echo '<script>alert("Inserted Successfully");window.location.assign("upload_pdf.php");</script>';
                                    } else{
                                        $error_message = mysqli_error($con);
                                        echo '<script>alert("Inserted Unsuccessfully: ' . $error_message . '");window.location.assign("upload_pdf.php");</script>'; 
                                    }
                                } else {
                                    echo '<script>alert("Error uploading file.");window.location.assign("upload_pdf.php");</script>';
                                }
                            } else {
                                echo '<script>alert("Invalid file type. Please upload a PDF or DOCX file.");window.location.assign("upload_pdf.php");</script>';
                            }
                        }
                        ?>
                                        </div>
